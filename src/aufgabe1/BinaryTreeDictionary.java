...
public class BinaryTreeDictionary<K, V> implements Dictionary<K, V>
{
	...
	private Node<K, V> insertR(K key, V value, Node<K, V> p)
	{
		...
		if (p == null)
		{
			p = new Node(new Entry(comparableKey, value), null, null);
			counter++;
			oldValue = null;
		}
		else if (comparableKey.compareTo(p.entry.getKey()) < 0)
		{
	...
	private Node<K, V> removeR(K unCompKey, Node<K, V> p)
	{
		Comparable key = (Comparable) unCompKey;
		...
		if (p == null)
		{
			...
			p.left = removeR(unCompKey, p.left);
		else if (key.compareTo(p.entry.getKey()) > 0)
			p.right = removeR(unCompKey, p.right);
